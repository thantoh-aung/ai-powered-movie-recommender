...
// Isolated card component so each card has its own Motion Values 
// and doesn't conflict during AnimatePresence unmounting in Vercel production
function SwipeCard({ movie, isTop, visualIndex, onSwipeLeft, onSwipeRight, onExplain, exitDirection }: any) {
        ...
        <motion.div
            ...
            variants={{
                exit: (customDirection: number) => ({
                    y: -100,
                    x: customDirection === 1 ? 500 : customDirection === -1 ? -500 : (x.get() > 0 ? 500 : -500),
                    opacity: 0,
                    rotate: customDirection === 1 ? 30 : customDirection === -1 ? -30 : (x.get() > 0 ? 30 : -30),
...
export default function SwiperUI({ movies, onLike, onDislike, onExplain }: any) {
    ...
    const handleLike = () => {
        ...
        setExitDirection(1); // Force right animation on unmount
    ...
    const handlePass = () => {
        if (currentIndex >= movies.length) return;
        setExitDirection(-1); // Force left animation on unmount
    ...
    if (currentIndex >= movies.length) {
        return (
            <div className="h-[600px] flex flex-col items-center justify-center text-[#9CA3AF] bg-white/5 backdrop-blur-md rounded-[20px] border border-white/10 shadow-lg w-full max-w-sm mx-auto">
